...
// Clock Domain Crossing (CDC) for ADC channels 2 and 3
// These channels are clocked by adc_clk_23, but consumed by logic on adc_clk_01.
// A 2-flop synchronizer is used to mitigate metastability.
// Note: This does not protect against data skew across the multi-bit bus,
// but is a necessary first step for stability. A more robust solution would
// be an asynchronous FIFO.
logic [14-1:0] adc_dat_raw_2_s1, adc_dat_raw_3_s1;
(* ASYNC_REG = "TRUE" *) logic [14-1:0] adc_dat_raw_2_s2, adc_dat_raw_3_s2;

always_ff @(posedge adc_clk_01, negedge adc_rstn_01) begin
    if (!adc_rstn_01) begin
        adc_dat_raw_2_s1 <= '0;
        adc_dat_raw_2_s2 <= '0;
        adc_dat_raw_3_s1 <= '0;
        adc_dat_raw_3_s2 <= '0;
    end else begin
        // Synchronizer for channel 2 data (from adc_clk_23 to adc_clk_01)
        adc_dat_raw_2_s1 <= adc_dat_raw[2];
        adc_dat_raw_2_s2 <= adc_dat_raw_2_s1;

        // Synchronizer for channel 3 data (from adc_clk_23 to adc_clk_01)
        adc_dat_raw_3_s1 <= adc_dat_raw[3];
        adc_dat_raw_3_s2 <= adc_dat_raw_3_s1;
    end
end
...
always @(posedge adc_clk_01) begin
  ...
  adc_dat_r[2] <= {adc_dat_raw_2_s2[14-1], ~adc_dat_raw_2_s2[14-2:0]};
  adc_dat_r[3] <= {adc_dat_raw_3_s2[14-1], ~adc_dat_raw_3_s2[14-2:0]};
  adc_dat      <= adc_dat_r;
end
...
red_pitaya_fads i_fads (
  ...
  .clk_i           (adc_clk_01 ),  // clock
  .rstn_i          (adc_rstn_01),  // reset - active low
  .dat_a_i         (adc_dat[0] ),  // in 1 from ADC
  .dat_b_i         (adc_dat[1] ),  // in 2 from ADC
  .dat_c_i         (adc_dat[2] ),  // in 3 from ADC
  .dat_d_i         (adc_dat[3] ),  // in 4 from ADC
  .sort_trig       (sort_trig  ),
